...
def check_api_keys() -> str | None:
    """Return error message if required API keys are missing."""
    if not os.getenv("MISTRAL_API_KEY"):
        return "MISTRAL_API_KEY is not set. Add it to your .env file or Streamlit secrets."
    return None
        ...
        if api_error:
            ...
            try:
                ...
                if not transcript or not transcript.strip():
                    raise ValueError("Transcription returned empty text. Please try a different video.")
...
if st.session_state.result:
    ...
    with chat_col1:
        user_input = st.text_input(
            ...
            key="chat_input"  # ← Fixed: added key to avoid Streamlit duplicate widget error
        )
    with chat_col2:
    ...
    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:5rem 2rem;text-align:center">
        ...
        <div style="color:var(--text-muted);font-size:0.85rem;max-width:380px;line-height:1.7">
            Paste a YouTube URL or local file path in the sidebar, choose your language,
            and hit <strong>Analyse</strong> to get started.
        </div>
